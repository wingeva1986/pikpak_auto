    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params, ssl=False) as response:
            ...
            print_json(json.dumps(pid, indent=4))
               image_bytes = requests.get(image_link,headers=headers).content

            data ={"image":base64.b64encode(image_bytes).decode(), "info": frames}
            jsonstr = json.dumps(data)
            data = base64.b64encode(jsonstr.encode()).decode()
            resp = requests.post(api_url, data=base64.b64encode(jsonstr.encode()).decode())
            select_id = resp.json()["text"]
